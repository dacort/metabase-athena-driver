...
(defmethod current-datetime-fn :athena [_] :%now)
...
(defmethod date [:athena :default] [_ _ expr] expr)
...
(defmethod field->identifier :athena [_ field]
...
(defmethod field->alias :athena [_ field]
  (:name field))
...
(defmulti quote-style
          ...
          statement. Defaults to `:ansi`, but other valid options are `:mysql`, `:athenaserver`, `:oracle`, and `:h2` (added in
...
(defmethod quote-style :athena [_] :ansi)
...
(defmethod unix-timestamp->timestamp [:athena :milliseconds] [driver _ expr]
...
(defmethod ->honeysql [:athena nil]    [_ _]    nil)
(defmethod ->honeysql [:athena Object] [_ this] this)

(defmethod ->honeysql [:athena :value] [driver [_ value]] (->honeysql driver value))

(defmethod ->honeysql [:athena :expression]
...
(defn get-field-identifier [field]
  (let [table            (qp.store/table (:table_id field))
    ...
    (if (nil? parent-id)
      ...
      (conj (handle-parent-identifier (->honeysql :athena [:field-id parent-id])) (:name field)))))

(defmethod ->honeysql [:athena (class Field)]
  [driver field]
  (let [field-identifier (get-field-identifier field)]
    (cast-unix-timestamp-field-if-needed driver field (keyword (apply hx/qualify-and-escape-dots field-identifier)))))

(defmethod ->honeysql [:athena :field-id]
  [driver [_ field-id]]
  (->honeysql driver (qp.store/field field-id)))

(defmethod ->honeysql [:athena :fk->]
...
(defmethod ->honeysql [:athena :field-literal]
  [driver [_ field-name]]
  (->honeysql driver (keyword (hx/escape-dots (name field-name)))))

(defmethod ->honeysql [:athena :datetime-field]
  [driver [_ field unit]]
  (date driver unit (->honeysql driver field)))

(defmethod ->honeysql [:athena :binning-strategy]
...
(defmethod ->honeysql [:athena :count] [driver [_ field]]
  (if field
    (hsql/call :count (->honeysql driver field))
    :%count.*))

(defmethod ->honeysql [:athena :avg]      [driver [_ field]] (hsql/call :avg            (->honeysql driver field)))
(defmethod ->honeysql [:athena :distinct] [driver [_ field]] (hsql/call :distinct-count (->honeysql driver field)))
(defmethod ->honeysql [:athena :stddev]   [driver [_ field]] (hsql/call :stddev         (->honeysql driver field)))
(defmethod ->honeysql [:athena :sum]      [driver [_ field]] (hsql/call :sum            (->honeysql driver field)))
(defmethod ->honeysql [:athena :min]      [driver [_ field]] (hsql/call :min            (->honeysql driver field)))
(defmethod ->honeysql [:athena :max]      [driver [_ field]] (hsql/call :max            (->honeysql driver field)))

(defmethod ->honeysql [:athena :+] [driver [_ & args]] (apply hsql/call :+ (map (partial ->honeysql driver) args)))
(defmethod ->honeysql [:athena :-] [driver [_ & args]] (apply hsql/call :- (map (partial ->honeysql driver) args)))
(defmethod ->honeysql [:athena :*] [driver [_ & args]] (apply hsql/call :* (map (partial ->honeysql driver) args)))
...
(defmethod ->honeysql [:athena :absolute-datetime]
  [driver [_ timestamp unit]]
  (date driver unit (->honeysql driver timestamp)))

(defmethod ->honeysql [:athena :time]
  [driver [_ value unit]]
  (date driver unit (->honeysql driver value)))

(defmethod ->honeysql [:athena :relative-datetime]
...
(defmethod apply-top-level-clause [:athena :aggregation]
...
(defmethod apply-top-level-clause [:athena :breakout]
...
(defmethod apply-top-level-clause [:athena :fields]
...
(defmethod ->honeysql [:athena :starts-with] [driver [_ field value options]]
  (like-clause driver (->honeysql driver field) (update-string-value value #(str % \%)) options))

(defmethod ->honeysql [:athena :contains] [driver [_ field value options]]
  (like-clause driver (->honeysql driver field) (update-string-value value #(str \% % \%)) options))

(defmethod ->honeysql [:athena :ends-with] [driver [_ field value options]]
  (like-clause driver (->honeysql driver field) (update-string-value value #(str \% %)) options))

(defmethod ->honeysql [:athena :between] [driver [_ field min-val max-val]]
  [:between (->honeysql driver field) (->honeysql driver min-val) (->honeysql driver max-val)])


(defmethod ->honeysql [:athena :>] [driver [_ field value]]
...
(defmethod ->honeysql [:athena :and] [driver [_ & subclauses]]
  (apply vector :and (map (partial ->honeysql driver) subclauses)))

(defmethod ->honeysql [:athena :or] [driver [_ & subclauses]]
  (apply vector :or (map (partial ->honeysql driver) subclauses)))

(defmethod ->honeysql [:athena :not] [driver [_ subclause]]
  [:not (->honeysql driver subclause)])

(defmethod apply-top-level-clause [:athena :filter]
...
(defmethod apply-top-level-clause [:athena :join-tables]
...
(defmethod apply-top-level-clause [:athena :order-by]
...
(defmethod apply-top-level-clause [:athena :limit]
  [_ _ honeysql-form {value :limit}]
  (h/limit honeysql-form value))

(defmethod apply-top-level-clause [:athena :page]
...
(defmethod ->honeysql [:athena (class Table)]
  [_ table]
  (let [{table-name :name, schema :schema} table]
    (hx/qualify-and-escape-dots schema table-name)))

(defmethod apply-top-level-clause [:athena :source-table]
